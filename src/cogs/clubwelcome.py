...
class ClubWelcome(commands.Cog):
    ...
    async def on_member_join(self, member):
        new_user_msg = (
            """Hi there! Welcome to the Knight Hacks Discord server.
            We're happy you're here!
            In order to finish joining:
            \t- Please accept the Code of Conduct using the ✅ reaction """
            """in the #code-of-conduct channel
            \t- Please set your nickname to match our naming rules.
            To check out the awesome ways to get involved, """
            """go to the #club-information channel.
            And be sure to introduce yourself in #introductions!"""
        )

        channel = self.bot.get_channel(LOGGING_CHANNEL)
        embed = discord.Embed(
            description=f"User {member.mention} joined the server.",
            color=0x00ff00
        )
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"Couldn't send welcome DM to {member.mention}",
                color=0xff0000
            )
            await channel.send(embed=embed)
    """
