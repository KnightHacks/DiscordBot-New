...
class AdministrationCommands(commands.Cog):
    ...
    """
        TO DO:
        Send "missing permissions" message
        when member who lacks permissions tries these commands.
    ...
    async def multiban(self, ctx,
        ...
        confirm = await ctx.prompt(
            f'This will ban **{total_members:member}** members. Are you sure?',
            reacquire=False
        )
        if not confirm:
        ...
        await ctx.send(
            f"Banned {total_members - failed}/{total_members} "
            f"members. {reason}"
        )
    ...
    async def unban(self, ctx, userID, reason=None):
        ...
        if not userID.isdigit():
            embed = discord.Embed(
                description=":x: Input a Valid User ID.",
                color=0xff0000
            )
            return await ctx.send(embed=embed)
        ...
        try:
            ...
            embed = discord.Embed(
                description=":white_check_mark: "
                f"**{userID.name}** has been unbanned.",
                color=0x00ff00
            )
            ...
            embed = discord.Embed(
                description=":x: Either this User ID is not valid or this "
                "user is not currently banned. Please input a valid user ID.",
                color=0xff0000
            )
            return await ctx.send(embed=embed)
    ...
    async def clear(self, ctx, amount=0):
        ...
        the bot must have manage messages permissions.
